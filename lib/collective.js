...
async function getOrCreateUserHash() {
  ...
  // Robust hash: UUID + installTime + SHA-256
  const uuid = crypto.randomUUID();
  const installTime = Date.now().toString();
  const raw = uuid + ':' + installTime;
  const encoder = new TextEncoder();
  const hashBuffer = await crypto.subtle.digest('SHA-256', encoder.encode(raw));
  const hashArray = Array.from(new Uint8Array(hashBuffer));
  const newHash = hashArray.map(b => b.toString(16).padStart(2, '0')).join('');

  await chrome.storage.local.set({ userHash: newHash, installTime });
  return newHash;
}
...
async function shareRelation(insight) {
  ...
  try {
    const result = await client.shareRelationSafe({
      p_user_hash: userHash,
      p_level_combo: clean.level,
      p_description: clean.description.substring(0, 280),
      p_c_before: clean.c_before,
      p_c_after: clean.c_after
    });

    if (result && result.error === 'RATE_LIMITED') {
      return { error: 'RATE_LIMITED', message: result.message };
    }
    return { success: true };
  } catch (err) {
...
async function getFullExport() {
  const [
    ...
    chrome.storage.local.get(['concepts']),
...
async function importFullExport(data) {
  ...
  if (data.node) {
    ...
    if (Array.isArray(data.node.concepts)) ops.concepts = data.node.concepts;
